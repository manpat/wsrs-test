...
void main() {
	...
	vec3 primary = normalize(mat3(normal_xform) * vec3(0.5, 0.0, -1.0));
	vec3 secondary = normalize(mat3(normal_xform) * vec3(1.0, 0.2, 0.0));
	...
	float ndotl = clamp(dot(vnormal, primary), 0.2, 1.0);
	float ndotl2 = clamp(dot(vnormal, secondary), 0.0, 1.0);

	// gl_FragColor = vec4(col * (ndotl * primcol + ndotl2 * seccol), 1.0);
	gl_FragColor = vec4(gamma_correct(col * (primcol * ndotl + seccol * ndotl2)), 1.0);
